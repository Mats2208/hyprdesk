...
- [ ] **Closing an agent doesn't kill its descendants.** We kill the direct child (the CLI); on Windows the **grandchildren survive**. Seen in the wild: a worker's Playwright left **four `chrome-headless-shell` processes** running, and because the page under test renders WebGL, they kept asking for frames **at 60fps for hours** — 80% of a Ryzen 9 9900X, burnt on a browser nobody was looking at. The same mechanism leaves orphaned `node` MCP servers behind. The fix is a **Job Object** on Windows (and a process group on Unix): when the PTY dies, the whole tree dies with it. Until then, a leaked headless browser is silent and expensive.
